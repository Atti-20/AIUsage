...
class TokenTally {
  ...
  TokenTally({this.input = 0, this.output = 0, this.cacheRead = 0, this.cacheWrite = 0, this.costUSD = 0});
...
class DailyStat {
  ...
  Map<String, dynamic> toJson() => {'day': day, 'claude': claude.toJson(), 'codex': codex.toJson()};
}
...
class ModelStat {
  ...
  factory ModelStat.fromJson(Map<String, dynamic> j) =>
      ModelStat(j['model'] as String, j['source'] as String, TokenTally.fromJson((j['tally'] as Map).cast()));

  Map<String, dynamic> toJson() => {'model': model, 'source': source, 'tally': tally.toJson()};
}
...
class ProjectStat {
  ...
  ProjectStat(this.name, this.path, this.source, this.tally, this.sessionCount, this.lastActivity);
  ...
  Map<String, dynamic> toJson() => {
        ...
        if (lastActivity != null) 'lastActivity': lastActivity!.toUtc().toIso8601String(),
      };
}
...
class SessionStat {
  ...
  SessionStat(this.sessionID, this.project, this.source, this.title, this.tally, this.lastActivity);
...
class LimitWindow {
  ...
  LimitWindow(this.key, this.label, this.utilization, this.resetsAt, this.windowMinutes);
...
class ClaudeLimits {
  ...
  ClaudeLimits({this.subscription, List<LimitWindow>? windows, this.fetchedAt, this.error})
      : windows = windows ?? [];

  factory ClaudeLimits.fromJson(Map<String, dynamic> j) => ClaudeLimits(
        subscription: j['subscription'] as String?,
        windows: ((j['windows'] as List?) ?? []).map((e) => LimitWindow.fromJson((e as Map).cast())).toList(),
...
class CodexLimits {
  ...
  CodexLimits({this.plan, List<LimitWindow>? windows, this.creditsBalance, this.capturedAt})
      : windows = windows ?? [];
...
class ResetStats {
  ...
  ResetStats(this.count, this.averageIntervalDays, this.longestIntervalDays, this.lastReset);
  ...
  static ResetStats compute(List<ResetEvent> events) {
    ...
    final avg = intervals.isEmpty ? null : intervals.reduce((a, b) => a + b) / intervals.length;
    final max = intervals.isEmpty ? null : intervals.reduce((a, b) => a > b ? a : b);
...
class UsageSnapshot {
  ...
  factory UsageSnapshot.fromJson(Map<String, dynamic> j) => UsageSnapshot(
        ...
        days: ((j['days'] as List?) ?? []).map((e) => DailyStat.fromJson((e as Map).cast())).toList(),
        models: ((j['models'] as List?) ?? []).map((e) => ModelStat.fromJson((e as Map).cast())).toList(),
        projects: ((j['projects'] as List?) ?? []).map((e) => ProjectStat.fromJson((e as Map).cast())).toList(),
        sessions: ((j['sessions'] as List?) ?? []).map((e) => SessionStat.fromJson((e as Map).cast())).toList(),
        claudeLimits: j['claudeLimits'] == null ? null : ClaudeLimits.fromJson((j['claudeLimits'] as Map).cast()),
        codexLimits: j['codexLimits'] == null ? null : CodexLimits.fromJson((j['codexLimits'] as Map).cast()),
        resets: ((j['resets'] as List?) ?? []).map((e) => ResetEvent.fromJson((e as Map).cast())).toList(),
      );
  ...
  double get monthCost {
    ...
    return days.where((d) => d.day.startsWith(prefix)).fold(0.0, (s, d) => s + d.totalCost);
  }

  double get totalCost => days.fold(0.0, (s, d) => s + d.totalCost);

  List<DailyStat> recentDays(int n) => days.length <= n ? days : days.sublist(days.length - n);
...
DateTime? parseDate(dynamic v) {
  ...
  if (v is num) return DateTime.fromMillisecondsSinceEpoch((v * 1000).toInt()).toLocal();
  return null;
}
